...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const openDiary = () => {
        ...
        setTimeout(() => {
            ...
            // Start typing animation after diary cover opens
            setTimeout(() => {
                startDiaryTypewriter();
            }, 800);
        }, 600);
    };

    const startDiaryTypewriter = () => {
        const bodyContainer = document.getElementById('diary-handwritten-body');
        const diarySign = document.getElementById('diary-sign');
        const scrollPage = bodyContainer ? bodyContainer.closest('.diary-page') : null;
        if (!bodyContainer) return;

        // Extract paragraphs text
        const paragraphs = Array.from(bodyContainer.querySelectorAll('p')).map(p => p.textContent.trim());
        bodyContainer.innerHTML = ''; // Clear existing text
        ...
        const typeNextChar = () => {
            if (pIndex >= paragraphs.length) {
                // Remove cursor and reveal signature
                cursor.remove();
                if (diarySign) {
                    diarySign.style.opacity = '1';
                }
                ...
                if (scrollPage) {
                    scrollPage.scrollTo({ top: scrollPage.scrollHeight, behavior: 'smooth' });
                }
                // Show next section button
                btnDiaryNext.classList.remove('btn--hidden', 'hidden');
            ...
            if (charIndex === 0) {
                ...
                bodyContainer.appendChild(currentP);
                currentP.appendChild(cursor);
            }

            const fullText = paragraphs[pIndex];

            if (charIndex < fullText.length) {
                const char = fullText.charAt(charIndex);
                currentP.insertBefore(document.createTextNode(char), cursor);
                charIndex++;

                // Auto-scroll logic: scroll down dynamically to keep current text visible
                if (scrollPage) {
                    scrollPage.scrollTo({
                        top: scrollPage.scrollHeight,
                        behavior: 'smooth'
                    });
                }

                // Variable typing delay for realistic handwriting feel
